...
function printUsage() {
 ...
 ${dim(`\u203A Press`)} r ${dim(`to restart packager, or`)} R ${dim(
			`to restart packager and clear cache.`
		)}
 ${dim(`\u203A Press`)} d ${dim(
			`to toggle development mode. (current mode: ${bold(
				devMode
			)}${chalk.reset.dim(')')}`
		)}
`
...
async function handleKeypress(key) {
	switch (key) {
		...
		case 'd':
			...
			log(
				`Packager now running in ${chalk.bold(
